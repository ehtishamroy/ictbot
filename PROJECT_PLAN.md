...
- **BUILD DONE — Part-D portfolio module complete & tested** (`src/ictbot/portfolio/`): data layer (ETF CSV loader + synthetic generator), `DEF-TSMOM-01` signal, `DEF-VOLSCALE-01` vol-scaled sizing (equal-risk + portfolio vol target + sleeve caps), monthly-rebalance backtester (no-trade band, cost accrual, cost_mult), portfolio metrics (Sharpe/Sortino/Calmar/DD/turnover) + **Deflated Sharpe Ratio** (no scipy). Validated on synthetic data: Sharpe scales with trend (0.05 noise → 0.28 → 0.59) and vol target holds ~10%. **82 tests green.**
- **Validation runner ready** (`scripts/run_divtrend_validation.py`): pre-registered protocol wired end-to-end (15-config IS grid ≤20 budget, single OOS look, cost-robustness-PRIMARY gate + Sharpe/stability/maxDD/breadth-leave-one-sleeve-out/DSR gates, Part F report). Smoke-tested on synthetic ETFs — runs clean, correctly caught a synthetic overfit as TESTED-FAIL.
- **ONE remaining need: DATA.** ≥24 liquid ETFs, daily adjusted-close, ≥15 years → drop CSVs in `data/raw/etf/` (see its README for the exact ticker list + format). Egress blocked here (Stooq/Yahoo 403), so user provides them (same flow as HistData). Then: `python3 scripts/run_divtrend_validation.py`.
- **RESULT: DIVTREND v1.0 is TESTED-PASS** on 19 ETFs, 2007-2026. All six pre-registered gates passed on a single OOS look, and it survived a hard post-pass robustness deep-dive (drop-2022 → 0.84, ±20% perturbation all positive, breadth-robust). Honest headline: **full-period Sharpe ~0.67** (~7%/yr at ~11% vol, ~19% maxDD) — a real, modest, cost-surviving, decayed-but-present trend edge. First TESTED-PASS in the whole project, on the one direction with real academic backing. `reports/DIVTREND_v1.0_findings.md`.
- **Next action: FORWARD TEST** (go-live ladder) — paper-trade the locked rules monthly for 6-12 months, confirm live behavior matches backtest, before any real capital; then small live (0.25% risk) and scale slowly. Backtest pass = first gate, not the finish line.
